...
def get_table_without_geometry(query, database):
...
def get_table_backup(query):
    conection = psycopg2.connect(database='Backup', user='docker', password='docker',
                                 host='localhost', port='25434')
    try:
        data = gpd.GeoDataFrame.from_postgis(query, conection, geom_col='geometry')
        conection.close()
...
def get_avg_atribute_from_neighbor(geometry, atribute):
    atribute = f'"{atribute}"'
    geometry = f"'{geometry}'"
    query = f'SELECT avg(a.{atribute}) from censal_full as a where ST_Intersects(a.geometry, ST_GeomFromText({geometry}, 4326))'
    return get_value(query)
...
def update_data_for_training():
    first_query = 'select p.*, c.neigh_density as density_neigh, c.pob8 as density from pollution p ' \
                  'join censal_full c on ST_Contains(c.geometry, p.geometry)'
    data = get_table(first_query)
...
def get_pollution_from_analysis_group_by_station():
    ...
    return get_table_analysis(query)
...
def compare_cluster_from_analysis(cluster1, cluster2, segmentation):
    ...
    engine = create_engine('postgresql://docker:docker@localhost:25434/Analysis', echo=False)
    try:
        data = pd.read_sql_query(query, con=engine)
        return data
    except Exception as ex:
...
def get_pollution_intersect(date, name, atribute):
    ...
    return get_value_analysis(query)


def make_grid(meters):  # Crea un cuadrado de puntos en andalucia con una distancia dada
    conection = psycopg2.connect(database='GeoHealth', user='docker', password='docker',
                                 host='localhost', port='25434')
    query = f'select st_transform(makegrid(st_transform(ST_Union(ST_SnapToGrid(a.geometry,0.000001)), 2062), {meters}), 4326) as geometry from health_districts_region a'
    try:
        data = gpd.GeoDataFrame.from_postgis(query, conection, geom_col='geometry')
    ...
    try:
        ...
        if geom:
            with engine.connect() as conn, conn.begin():
                sql = f'ALTER TABLE {name_table} ALTER COLUMN geometry TYPE Geometry USING ST_SetSRID(geometry::Geometry, 4326)'
                conn.execute(sql)
